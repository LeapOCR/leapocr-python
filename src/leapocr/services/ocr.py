...
import asyncio
from typing import Optional, Union, Dict, Any, List, AsyncIterator, Iterator
from pathlib import Path
import io
from datetime import datetime

from ..client import LeapOCRClient
from ..exceptions import LeapOCRClientError, LeapOCRTimeoutError, LeapOCRJobError
from ..types import (
    ProcessFormat,
    ProcessTier,
    Job,
...
class OCRService:
    ...
    async def process_document_async(
        ...
        if isinstance(source, (str, Path)):
            source_str = str(source)
            if source_str.startswith(('http://', 'https://')):
                return await self._process_url_async(source_str, options, wait_for_result, timeout)
            else:
                return await self._process_file_async(source, options, wait_for_result, timeout)
        else:
            # File-like object
            return await self._process_file_async(source, options, wait_for_result, timeout)
    ...
    async def batch_process_async(
        ...
        async def process_single(source: Union[str, Path, io.IOBase]) -> Union[Job, JobResult]:
    ...
    def extract_text(
        ...
        result = self.process_document(source, options, wait_for_result=True, timeout=timeout)
