...
from .gen.openapi_client import LeapOCRGenClient, Configuration
...
class LeapOCRClient:
    ...
    def __init__(
        ...
        self.user_agent = user_agent or f"leapocr-python/0.1.0"
    ...
    def _handle_api_error(self, error: ApiException) -> None:
        ...
        if status_code == 401:
            ...
            raise LeapOCRAuthenticationError("Access forbidden - check API key permissions")
    ...
    def process_url(
        ...
        try:
            ...
            format_str = format.value if isinstance(format, ProcessFormat) else str(format)
            tier_str = tier.value if isinstance(tier, ProcessTier) else str(tier)
            
            from .gen.openapi_client.models.upload_url_upload_request import UploadURLUploadRequest
    ...
    def upload_file(
        ...
        try:
            ...
            from .gen.openapi_client.models.upload_initiate_upload_request import UploadInitiateUploadRequest
            
            initiate_request = UploadInitiateUploadRequest(
                format=format.value if isinstance(format, ProcessFormat) else str(format),
            ...
            initiate_response = self._gen_client.presigned_upload_with_http_info(initiate_request)
    ...
    def wait_for_result(
        ...
        while True:
            ...
            if timeout and (time.time() - start_time) > timeout:
                raise LeapOCRTimeoutError(f"Job {job_id} did not complete within {timeout} seconds")
