...
class SimpleLeapOCRClient:
    ...
    def __init__(self, api_key: str, base_url: str = "http://localhost:8080"):
        ...
        self.client = httpx.Client(
            timeout=30.0,
            headers={"X-API-Key": api_key, "User-Agent": "leapocr-python-test/0.1.0"},
        )

    def process_url(
        self, url: str, format: str = "structured", tier: str = "core"
    ) -> Dict[str, Any]:
        """Process a document from URL."""
        endpoint = f"{self.base_url}/api/v1/ocr/uploads/url"

        payload = {"url": url, "format": format, "tier": tier}
        ...
        while time.time() - start_time < max_wait:
            try:
                ...
                if status == "completed":
                    ...
                    if result.get("data"):
                        print(
                            f"   Extracted data preview: {str(result.get('data'))[:200]}..."
                        )
                    break
                elif status == "failed":
                    print(f"❌ Job failed!")
                    ...
                    print(
                        f"⏳ Job status: {status} (progress: {result.get('progress_percentage', 0)}%)"
                    )
                    time.sleep(5)
